...
/// Flat-config ESLint setup that turns on `eslint-plugin-jsx-a11y` in
/// recommended-strict mode. Required as a CI gate by the 2026-05-17
/// Section 508 audit (F-028). Owned: refresh by tweaking the vendor
/// constant and regenerating.
pub const ESLINT_CONFIG_JS: &str = r#"// Generated by schema-forge — edit freely.
//
// Accessibility-first lint baseline. eslint-plugin-jsx-a11y catches the
// regression classes flagged in the 2026-05-17 Section 508 audit
// (placeholder-as-label, alt text on decorative images, label/control
// association, etc.) before they reach a Playwright + axe gate.
...
/* ---- Confirmation dialog (Radix AlertDialog skin) ---- */
.confirm-overlay {
  position: fixed;
  inset: 0;
...
.confirm-content:focus { outline: none; }
.confirm-title {
  margin: 0 0 8px;
  font-size: 15px;
  font-weight: 600;
...
/// Vendored file-upload primitives used by per-entity edit and detail pages
/// when their schema declares a `file` field. Two exports:
///
/// * `FileUpload` — controlled editor that drives the three-endpoint flow
///   (mint presigned URL → PUT bytes to object storage → confirm upload).
///   Renders as a disabled "save first" hint when `entityId` is undefined,
///   so create-mode forms still mount.
/// * `AttachmentDownload` — read-only widget for detail pages. Honors the
///   field's `access` mode: `presigned` issues a JSON `?redirect=false`
///   round-trip then opens the URL; `proxied` links the field endpoint
///   directly so the runtime can stream bytes through with auth applied.
///
/// Both components are self-contained (read `VITE_API_BASE` and the auth
/// token themselves) so per-entity templates only need to wire schema name,
/// entity id, field name, and field metadata.
pub const FILE_UPLOAD: &str = r##"// Generated by schema-forge — edit freely.
//
// File upload + download primitives shared across entities that declare
// `file` fields. The runtime exposes a three-endpoint upload flow:
//
//   1. POST /api/v1/forge/schemas/<schema>/entities/<id>/fields/<field>/upload-url
//   2. PUT <presigned_url>            (bytes, no auth, direct to storage)
//   3. POST .../fields/<field>/confirm-upload
//
// `FileUpload` drives that flow as a controlled component for use inside a
...
/** Persisted attachment shape — matches the runtime's `confirm-upload` JSON. */
...
/** Per-field metadata projected from the schema's `file(...)` constraints. */
export type FileFieldMeta = {
  maxSizeBytes: number
  maxSizeHuman: string
  mimeAllowlist: string[]
  /** "presigned" issues a 302 to the storage URL; "proxied" streams via the daemon. */
  access: "presigned" | "proxied"
}

type FileUploadProps = {
  /** Schema name as it appears in the REST path (e.g. `"Document"`). */
  schema: string
  /** Entity id; `undefined` in create mode (form has not been saved yet). */
  entityId: string | undefined
  /** Field name on the entity. */
  fieldName: string
  /** Field metadata from the schema (limits + access mode). */
  meta: FileFieldMeta
  /** React Hook Form's controlled value. */
  value: FileAttachment | null | undefined
  /** React Hook Form's onChange — receives the next attachment or null. */
  onChange: (next: FileAttachment | null) => void
  /** Disable inputs (e.g. while a parent mutation is in flight). */
  disabled?: boolean
}

/**
 * Build the `/api/v1/forge/schemas/<schema>/entities/<id>` base URL for a
 * given entity. Returns `null` when the entity has not been created yet,
 * which short-circuits create-mode rendering to the "save first" hint.
 */
function entityUrlOrNull(schema: string, entityId: string | undefined): string | null {
  if (!entityId) return null
  return `${API_BASE}${FORGE_API_PREFIX}/schemas/${encodeURIComponent(
    schema,
...
/**
 * Controlled file-upload widget for a `file`-typed field. Drives the
 * three-endpoint flow against the entity URL derived from `schema` + `entityId`.
 *
 * Create mode (entityId undefined) is intentionally disabled — uploads need
 * an existing entity id so the runtime can attach the confirmation. The
 * widget renders a hint asking the user to save first; the form itself still
 * mounts and other fields stay editable.
 */
export function FileUpload({
  ...
  const entityUrl = entityUrlOrNull(schema, entityId)

  async function handleFile(file: File) {
    if (!entityUrl) return
    setErr(null)
    if (meta.maxSizeBytes && file.size > meta.maxSizeBytes) {
      setErr(`File exceeds ${meta.maxSizeHuman}.`)
      return
    }
    setUploading(true)
    setProgress(0)
    try {
      ...
      const mintRes = await fetch(`${entityUrl}/fields/${fieldName}/upload-url`, {
        method: "POST",
      ...
      // Step 2 — PUT bytes via XHR so we can surface a progress bar.
      // (Fetch streams have wider browser support now but progress events
      // through XHR remain the simplest cross-browser path.)
      ...
      const confirmRes = await fetch(`${entityUrl}/fields/${fieldName}/confirm-upload`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders() },
  ...
  // Create mode: no entity id yet, so uploads cannot be confirmed.
  if (!entityUrl) {
    return (
      <div
        className="mono"
        style={{
          ...
          borderRadius: 4,
          color: "var(--app-fg-3, #666)",
        }}
      >
        Save this {schema.toLowerCase()} first, then upload a file from the edit page.
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      {value?.key ? (
        ...
        <input
          type="file"
          accept={meta.mimeAllowlist.length > 0 ? meta.mimeAllowlist.join(",") : undefined}
          disabled={disabled || uploading}
          onChange={(ev) => {
      ...
      <p className="help mono" style={{ fontSize: 11 }}>
        Up to {meta.maxSizeHuman}.
...
type AttachmentDownloadProps = {
  ...
  access: "presigned" | "proxied"
  /** Render as a small inline link instead of a full button. */
  compact?: boolean
}

/**
 * Read-only download primitive used by detail pages. Behavior depends on the
 * field's `access` mode:
 *
 * * `presigned` — fetches `?redirect=false` to receive a JSON envelope with a
...
export function AttachmentDownload({
  ...
  const filename = attachment.key.split("/").pop() || attachment.key
  const fieldUrl = `${API_BASE}${FORGE_API_PREFIX}/schemas/${encodeURIComponent(
    schema,
...
/// React hook that sets the document title for the current route.
///
/// Federal accessibility audit (Section 508 / WCAG 2.1 SC 2.4.2 Page
/// Titled) flagged that every route reported the same hardcoded
/// `<title>site</title>`. Screen-reader page-title navigation, browser
...
///
/// Federal accessibility audit (Section 508 / WCAG 2.1) required us to
/// replace `window.confirm` for destructive actions: AT support is
/// inconsistent, focus return after dismissal is non-deterministic, the
/// browser-native dialog cannot be styled to match focus/contrast tokens,
/// and several screen-magnifier products lose the dialog entirely.
///
/// Radix AlertDialog gives us role="alertdialog", aria-labelledby +
/// aria-describedby on title and description, focus trap, Escape-to-cancel,
/// and focus restoration to the trigger on close — out of the box.
pub const CONFIRM_DIALOG: &str = r#"// Generated by schema-forge — edit freely.
import * as React from "react"
import { AlertDialog } from "radix-ui"
import { Button } from "@/components/ui/button"

export interface ConfirmDialogProps {
  /** The trigger element (e.g. a <Button>). Rendered via Slot. */
  trigger: React.ReactNode
  /** Title — becomes the dialog's accessible name. */
  title: string
  /** Body text — becomes the dialog's accessible description. */
  description: React.ReactNode
  /** Label on the primary confirm button. Defaults to "Confirm". */
  confirmLabel?: string
  /** Label on the cancel button. Defaults to "Cancel". */
  cancelLabel?: string
  /** When true, the confirm button takes destructive styling. */
  destructive?: boolean
  /** When true, the confirm button is disabled (e.g. while pending). */
  busy?: boolean
...
/**
 * Accessible confirmation surface. Wrap the action's button as `trigger`:
 *
 *   <ConfirmDialog
 *     trigger={<Button variant="ghost" size="xs">Delete</Button>}
 *     title="Delete record?"
 ...
 *   />
 *
 * Cancel button receives initial focus per the alertdialog pattern so a
 * stray Enter keypress does not destroy data.
 */
