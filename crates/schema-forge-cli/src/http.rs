...
pub fn forge_base(server: &str, api_version: &str) -> String {
    format!("{}/api/{}/forge", server.trim_end_matches('/'), api_version)
}
...
impl ForgeClient {
    ...
    pub async fn list(&self, schema: &str, query: &[(String, String)]) -> Result<Value, CliError> {
    ...
    pub async fn put_file_bytes(
        ...
        let snippet = resp.text().await.map(|t| snippet(&t)).unwrap_or_default();
    ...
    pub async fn get_export_job(&self, schema: &str, job_id: &str) -> Result<Value, CliError> {
...
mod tests {
    ...
    fn classify_422_folds_in_details() {
        let body = r#"{"error":"validation_failed","message":"2 invalid","details":["name required","age must be > 0"]}"#;
    ...
    fn classify_falls_back_to_raw_body() {
        ...
        let CliError::Http { kind, message, .. } = err else {
            panic!("expected Http");
        };
