...
schema-forge serve --no-console                             # disable the embedded ops console at /console
```

Environment variables for admin: `FORGE_ADMIN_USER`, `FORGE_ADMIN_PASSWORD`.

The HTMX site surface was removed in commit `fdd4976`. The site UI is now a separate React + Vite + Tailwind + shadcn project generated by `schema-forge site generate`. The backend serves the REST API, auth endpoints, and (since v0.34.0) an embedded operator console at `/console` when the cli is built with the default `embedded-console` feature; pass `--no-console` to disable it. Console sources live in the separate `schemaforge-console` repo.

Backend selection is by cargo feature (`surrealdb` default, `postgres`, `mssql`) plus `[database] url` in config — see config-reference.md for the SQL Server / Windows integrated auth form.
